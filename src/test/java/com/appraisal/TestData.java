...
public class TestData {
    public static AddEmployeeModel generateEmployeeModelRequest(){
        ...
        return AddEmployeeModel.builder().firstName("Test").lastName("Test").emailAddress("test@test.com").dateEmployed(now).build();
    }

    public static Employee generateEmployee(){
        LocalDate now = LocalDate.now();
        LocalDateTime localDateTime = now.atStartOfDay();
        Employee employee = Employee.builder().firstName("Test").lastName("Test").emailAddress("test@test.com").dateEmployed(localDateTime).build();
        employee.setId(1L);
        return employee;
    }
    ...
    public static AddEmployeeModel generateEmployeeModelRequestWithManager(){
        LocalDate now = LocalDate.now();
        return AddEmployeeModel.builder().firstName("Test").lastName("Test").emailAddress("test@test.com").dateEmployed(now).managerId(1L).build();
    }
